...
ЗАДАЧА: на основе ответов сотрудника составить аккуратный структурированный итог дня.

КРИТИЧЕСКИ ВАЖНО: НИЧЕГО НЕ ВЫДУМЫВАЙ. Используй ТОЛЬКО факты, которые написал сотрудник. Запрещено добавлять детали, действия, достижения или выводы, которых нет в ответах. Если по блоку нет информации — не включай его. Нельзя приукрашивать или додумывать.

ЧТО МОЖНО: переформулировать корявые предложения в грамотный деловой язык, сгруппировать по проектам, убрать повторы, исправить орфографию.

ЛИЦО ПОВЕСТВОВАНИЯ: пиши строго от ПЕРВОГО лица единственного числа, как пишет сам сотрудник о себе ("проверил", "сделал", "разобрался"). НЕ пиши от третьего лица. Отглагольное существительное ("разработка плагина") переформулируй в первое лицо ("разрабатывал плагин") или оставь как у сотрудника — смотря как правильнее в контексте.

ЗАПРЕЩЕНО ИСПОЛЬЗОВАТЬ MARKDOWN И ЛЮБЫЕ СПЕЦСИМВОЛЫ ФОРМАТИРОВАНИЯ. НЕ используй звёздочки (*), решётки (#), подчёркивания (_), обратные кавычки. Никакого **жирного** или # заголовков. Только чистый текст.

ФОРМАТ ВЫВОДА — строго так:
- Заголовок раздела пиши обычным текстом с двоеточием в конце, с новой строки.
- Каждый пункт начинай с символа • (кружок) и пробела.
- Между разделами оставляй ОДНУ пустую строку.
- Каждый пункт и каждый заголовок — с новой строки (перенос строки).

Пример правильного формата:

Проекты:
• Школа ВОГ: завершил работу и передал файлы.
• ЖК Нагатинский: выполнил зоны и ТЭП.

Трудности и замечания:
• Возникла трудность с переустановкой Revit.

Координация, встречи, прочее:
• Участвовал во встрече в телемосте в 10:00 с Дмитрием.

РАЗДЕЛЫ (включай только те, по которым есть информация):
Проекты: (с группировкой — по каждому проекту свои пункты)
Трудности и замечания:
Что нового узнал:
Координация, встречи, прочее:

СТИЛЬ: деловой, по пунктам. Без эмодзи. Без вводных фраз вроде "вот итог". Сразу содержание.

ДЛИНА: объём итога прямо пропорционален объёму ответов сотрудника. Чем подробнее ответы — тем длиннее и детальнее итог. Не раздувай искусственно и не сокращай то, что написал сотрудник."""
...
def clean_markdown(text: str) -> str:
    """Подстраховка: убирает markdown-символы, которые WEEEK не понимает."""
    import re
    # убираем ** и __ (жирный), * и _ одиночные на границах слов, # заголовки, ` кавычки
    text = text.replace("**", "").replace("__", "")
    text = re.sub(r"^#{1,6}\s*", "", text, flags=re.MULTILINE)  # # заголовки в начале строк
    text = text.replace("`", "")
    # markdown-маркеры списков (* или - в начале строки) -> заменяем на bullet
    text = re.sub(r"^\s*[\*\-]\s+", "• ", text, flags=re.MULTILINE)
    return text.strip()


async def generate_ai_report(answers: dict):
...
HELP_TEXT = (
    ...
    "«Создать итог дня» — задам 5 вопросов, оформлю ответы с помощью ИИ "
    "(ничего не выдумываю), покажу итог и спрошу, отправить ли его в WEEEK "
    "в твою колонку.\n\n"
    "«Сменить пользователя» — заново выбрать свою фамилию.\n\n"
    "Команды: /report, /cancel."
)
