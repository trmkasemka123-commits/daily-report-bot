...
MIN_ANSWER_LENGTH = 50  # порог символов
FOLLOWUP_TEXT = "Ответ короткий. Расскажи, пожалуйста, чуть подробнее — так итог получится полнее и информативнее:"
...
async def reformat_report(text: str):
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(DEEPSEEK_URL, headers=headers, json=payload) as resp:
                if resp.status != 200:
                    logger.error(f"DeepSeek reformat статус {resp.status}: {(await resp.text())[:300]}")
                    return None
                data = await resp.json()
                content = data["choices"][0]["message"]["content"].strip()
                return clean_markdown(content)
    except Exception as e:
        logger.error(f"DeepSeek reformat ошибка: {e}")
        return None
...
async def on_send_weeek(call: CallbackQuery, state: FSMContext):
    ...
    if ok:
        ...
        await call.message.answer("Хочешь перейти в WEEEK для просмотра итога дня?", reply_markup=main_menu())
    else:
        await call.message.answer(
...
async def _finalize_and_send(message: Message, state: FSMContext, report_text: str):
    ...
    if ok:
        ...
        await message.answer("Хочешь перейти в WEEEK для просмотра итога дня?", reply_markup=main_menu())
    else:
        await message.answer(
...
async def on_edited_text(message: Message, state: FSMContext):
    """Принимает отредактированный текст, переразмечает через DeepSeek и шлёт в WEEEK."""
    ...
    # переразметка через DeepSeek (не меняя содержания)
    ...
    # подстраховка: если DeepSeek всё же вернул шапку — вырезаем ещё раз
