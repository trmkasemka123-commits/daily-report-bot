...
def main_menu() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text=BTN_NOTES_REPORT)],
...
HELP_TEXT = (
    ...
    "Заметки хранятся до 8:00 следующего дня. Если утром ты не отправил итог, "
    "но заметки есть — в 9:00 я сам покажу собранный итог для отправки.\n\n"
    "👤 Сменить пользователя — заново выбрать фамилию.\n\n"
    "Команды: /report — начать опрос, /cancel — прервать."
)
...
async def on_note_text(message: Message, state: FSMContext):
    ...
    if ok:
        ...
        await message.answer("Чтобы добавить ещё — нажми «📝 Заметка».", reply_markup=main_menu())
    else:
        await message.answer(
...
async def auto_show_notes_reports():
    """9:00 МСК: тем, у кого есть заметки за сегодня и кто ещё НЕ сдал итог,
...
def setup_scheduler(bot):
    ...
    # автопоказ итогов из заметок — каждый день в 9:00 МСК
    scheduler.add_job(
    ...
    scheduler.start()
    logger.info("Планировщик запущен: напоминания 18:30 (пн-пт), автопоказ 9:00, очистка заметок 8:00 (МСК)")
