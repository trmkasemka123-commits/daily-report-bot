...
1. Для КАЖДОГО проекта — ОТДЕЛЬНЫЙ раздел. Заголовок раздела пиши КАПСОМ (заглавными буквами): РАБОТА НАД [НАЗВАНИЕ ПРОЕКТА]. Под ним пункты с • по этому проекту. НЕ объединяй разные проекты в один раздел — каждый проект это свой раздел со своим капс-заголовком.

2. Если сотрудник написал про трудности — отдельный раздел с заголовком капсом: ТРУДНОСТИ.
...
ФОРМАТ КАЖДОГО РАЗДЕЛА (соблюдай переносы строк ТОЧНО так):
- Заголовок раздела КАПСОМ на отдельной строке.
- Далее каждый пункт с новой строки, начинается с "• ".
- После последнего пункта раздела — ПУСТАЯ СТРОКА, затем следующий раздел.

ОБЯЗАТЕЛЬНО разделяй разделы пустой строкой и каждый заголовок начинай с НОВОЙ СТРОКИ. Не склеивай конец одного раздела с заголовком следующего.
...
def clean_markdown(text: str) -> str:
    """Подстраховка: убирает markdown и нормализует переносы строк."""
    import re
    # убираем markdown-жирный, заголовки, кавычки
    text = text.replace("**", "").replace("__", "")
    text = re.sub(r"^#{1,6}\s*", "", text, flags=re.MULTILINE)
    text = text.replace("`", "")
    # markdown-маркеры списков (* или - в начале строки) -> bullet
    text = re.sub(r"^\s*[\*\-]\s+", "• ", text, flags=re.MULTILINE)
    # ПОДСТРАХОВКА ОТ СЛИПАНИЯ: если пункт заканчивается, а сразу за ним КАПС-заголовок
    # без переноса (напр. "работу.ТРУДНОСТИ") — вставим перенос.
    # Находим: точка/буква, сразу заглавная кириллица в составе КАПС-слова.
    # Заголовки у нас капсом, поэтому ставим двойной перенос перед строкой,
    # которая целиком из заглавных (заголовок раздела).
    lines = text.split("\n")
    fixed = []
    for line in lines:
        stripped = line.strip()
        # это заголовок-капс? (буквы заглавные, нет •, длиннее 2 символов)
        ...
        if is_caps_header and fixed and fixed[-1].strip() != "":
            fixed.append("")  # пустая строка перед заголовком
        fixed.append(line)
    text = "\n".join(fixed)
    # схлопываем тройные+ переносы до двойных
    text = re.sub(r"\n{3,}", "\n\n", text)
    return text.strip()
