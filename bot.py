...
def show_notes_keyboard() -> InlineKeyboardMarkup:
    """Inline-кнопка 'Показать' заметки за день."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Показать", callback_data="show_notes")],
    ])
...
async def btn_note(message: Message, state: FSMContext):
    """Нажатие 'Заметка' — ждём одно сообщение с текстом заметки."""
...
async def on_note_text(message: Message, state: FSMContext):
    ...
    # если нажали кнопку меню вместо текста заметки — выходим из режима
    ...
    if ok:
        await message.answer(
            f"Записал! Заметок за день: {count}. Показать заметки за день?",
            reply_markup=show_notes_keyboard(),
        )
    else:
        await message.answer(
...
async def on_show_notes(call: CallbackQuery):
    """Показать накопленные заметки за день."""
    today = moscow_today()
    notes = await storage.get_notes(call.from_user.id, today)
    await call.answer()
    if not notes:
        await call.message.answer("За сегодня заметок пока нет.")
        return
    lines = [f"{i+1}. {n}" for i, n in enumerate(notes)]
    await call.message.answer("Твои заметки за день:\n\n" + "\n".join(lines))
